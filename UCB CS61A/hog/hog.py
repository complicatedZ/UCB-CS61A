...
def make_averaged(original_function, trials_count=1000):
    ...
    # BEGIN PROBLEM 8
    "*** YOUR CODE HERE ***"
    def average_turns(*args):
        result = sum([original_function(*args) for i in range(trials_count)])
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    zero_get_score = free_bacon(opponent_score)
    if (is_swap(score + zero_get_score, opponent_score)):
        if(opponent_score > score + zero_get_score):
            return 0
        else :
            return num_rolls
    else:
