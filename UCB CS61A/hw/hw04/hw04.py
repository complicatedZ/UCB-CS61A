...
def has_path(t, phrase):
    ...
    if label(t) != phrase[0]:
        return False
    elif (is_leaf(t) and label(t) == phrase[0]) or (label(t) == phrase[0] and len(phrase) == 1):
        return True
    else:
        for b in branches(t):
            if label(b) == phrase[1]:
...
def quadratic(x, a, b, c):
    ...
    if a > 0:
        if lower <= symmetric_axis <= upper:
            return interval(f(symmetric_axis), max(f(lower), f(upper)))
        else:
            return interval(min(f(lower), f(upper)), max(f(lower), f(upper)))
    else:
        if lower <= symmetric_axis <= upper:
            return interval(min(f(lower), f(upper)), f(symmetric_axis))
        else:
            return interval(min(f(lower), f(upper)), max(f(lower), f(upper)))
