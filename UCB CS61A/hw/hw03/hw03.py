...
def missing_digits(n):
    ...
    >>> check(pyHW_SOURCE_FILE, 'missing_digits', ['While', 'For'])
    True
    """
    "*** YOUR CODE HERE ***"
    if n < 10:
        ...
        miss = (n % 10) - ((n // 10) % 10) - 1
        if miss > 0:
            return missing_digits(n // 10) + miss
        else:
...
def count_change(total):
    ...
    def count_helper(total, small_coin):
        if total == 0:
            ...
            return count_helper(total, small_coin * 2) + count_helper(total - small_coin, small_coin)
...
def make_anonymous_factorial():
    ...
    True
    """
    return lambda n : (lambda x, function : 1 if x == 1 else mul(x, function(sub(x, 1), function)))(n, lambda x, function : 1 if x == 1 else mul(x, function(sub(x, 1), function)))
